...
- 后台保存文章时会自动整理常见 Markdown 结构：补齐段落空行、将类似 `1.Agent` 的编号标题规范为标题、将独立的“优点：”类标签整理为小标题；代码围栏和表格会作为完整结构保护，CMS 误加的 `\~\~\~` 围栏转义也会自动恢复。部署前还会再次规范化并校验，无法安全判断的情况会给出具体行号。
- 文章编辑页提供可选的文章 `slug` 字段。留空时沿用文件名；填写时只能使用小写英文、数字和短横线，例如 `prompt-engineering`。它只影响当前文章的 URL，与板块注册表中的板块 `slug` 是两套字段；构建检查会拒绝同一板块内的重复文章路由。修改已有文章的 slug 会改变 URL，如需保留旧链接应另行配置 Hugo alias。
