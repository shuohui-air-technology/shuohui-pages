...
我们可以从这个例子中直观的看到更加“**详细**”的提示词是什么影响模型的输出的。
...
### 这就是**提示词工程**的应用之一，在这里我们给出提示词工程的如下定义

**围绕模型输入进行设计、组织、测试和迭代，使模型在特定任务中产生更符合目标的输出**

_通常而言，这样的设计可以是明确需求，拆解任务，指明模型使用的工具，解决问题的方式，或者更多其余有利模型进行任务的输入设计_

提示词里可以包含任务目标、背景材料、示例、角色、约束、输出格式、工具说明和验收标准

在聊天窗口中，它通常是一段自然语言；在一个完整的 AI 应用中，它还会和上下文、工具、模型参数、状态管理以及评测系统一起工作
...
### 为了让你更好的理解为什么要有提示词工程，让我们考虑这样一件事情

### _它被设计出来是为了什么？_

说得简单一些，提示词工程最初关注的是这样一件事
...
所以，在ai 发展的早期阶段，提示词承担了一个很实际的作用：它把一个已经冻结的模型临时接到一个新任务上。模型从预训练数据中获得了语言模式和知识，提示词负责把这些能力引向当前任务。
...
任务描述是传统提示词中最容易理解的一部分。它主要负责把一个宽泛的请求改写成一个可以执行的动作。
...
在这条链中，一次简单问答可能只需要第一层；但一个可重复运行的智能体系统则可能会同时涉及全部层级。
...
所以，提示词工程目前事实上处于一个不断上移的过程中。它最初讨论的是怎样选择词语、怎样安排示例、怎样设计格式；现在讨论的是需求怎样被表达，上下文怎样被组织，模型怎样调用工具，输出怎样被检查。

现在的 ai 迭代是如此之快，我们现有的很多在其发展初期学习的技术对初学者而言已经几乎没有学习的必要了，因为随着模型的发展，它将本身就具备这样的能力

我相信提示词工程或许就属于这样的技术，它也迟早将变得无关紧要。事实上，对我而言，我已经几乎记不清上一次坐在电脑前绞尽脑汁组织提示词是什么时候，现在的交互体验变得越来越自然，ai 几乎轻而易举就能理解我的需求和想法，这在某种程度上让我感到满意，也感到某种焦虑，这两者出于相同的原因。

但对于阅读到此的你而言，对它的理解仍然是必要的，这种理解将在悄无声息中变成一种本能。
